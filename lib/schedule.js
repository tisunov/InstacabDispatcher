...
module.exports = {
	...
	_isOutOfSchedule: function() {
		...
		if (range1 && range2) {
			outOfScheduleRequest = (hourOfDay < range1.start || hourOfDayEnd > range1.end) && (hourOfDay < range2.start || hourOfDayEnd > range2.end);
		}
		else {
			outOfScheduleRequest = hourOfDay < range1.start || hourOfDayEnd > range1.end;
		}
